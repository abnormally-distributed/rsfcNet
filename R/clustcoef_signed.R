...
#'
#' @example
#' CC = clustcoef_signed_mult(graphs)
#'
#' @references
#' \href{https://sites.google.com/site/bctnet/measures/list}{Brain Connectivity Toolbox}
#'
...
clustcoef_signed_mult = function(graphs,method="Constantini") {

  if (method=="Constantini") {
    ltr = pbapply::pbsapply(graphs, function(g) clustcoef_signed(g, method=method))
    ltr = t(ltr)
    return(ltr)
  } else  if (method=="Zhang") {
    ltr = pbapply::pbsapply(graphs, function(g) clustcoef_signed(g, method=method))
    ltr = t(ltr)
    colnames(ltr) = col.names
    rownames(ltr) = row.names
    return(ltr)
  }
}
