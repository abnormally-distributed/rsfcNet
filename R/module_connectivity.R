...
#' @param connectivity_matrix a single connectivity matrix .
#' @param modules The list of communities objects from an igraph communities detection function.
...
#'
#' @details This calculates the (within-module) strength star and diversity star metrics proposed by Rubinov and Sporns (2011). Also returns Within-Module Z-Score, Within-Module Pos/Neg Strength, Diversity-Star, Participation Coefficient, and percentage of total connections that exist within-module. Z-Scores and Participation Coefficient measures are based on the strength (star) of the nodes. In the event a binary network is fed, diversity will not be calculated. Participation will be the more appropriate measure. The strength in the output for a binary graph is just the degree.
#'
#' @examples
#' module_statistics = module_connectivity(cormat, module_labels)
#'
#' @references
#'
...
#'
module_connectivity = function(connectivity_matrix, modules, scale=FALSE, n.nodes=.GlobalEnv$n.nodes) {
  matrix.mod = connectivity_matrix
  diag(matrix.mod) = 0
  ...
  strength_modules = lapply(module_matrices, function(m) strength_star(m, scale=scale))
  ...
  strength_star = strength_star(connectivity_matrix, scale = scale)
  ...
  `% of Total Conn.` = sorted.net$module.star/sorted.net$star.network

  participation.stats = cbind.data.frame(module.strength=sorted.net$module.star, network.strength=sorted.net$star.network,`% of Total Conn.`, participation)
