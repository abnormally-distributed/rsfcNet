...
#'
#' @examples
#' leverage_centr_single(binary.graph, weighted=FALSE)
#' leverage_centr_single(weighted.graph, weighted=TRUE)
#'
#' @seealso \code{\link{rsfcNet::leverage_centr}}
#'
#' @references
...
leverage_centr_single = function(graph, weighted=FALSE){
  if (weighted==TRUE){
  k <- strength(graph)
  n <- vcount(graph)
